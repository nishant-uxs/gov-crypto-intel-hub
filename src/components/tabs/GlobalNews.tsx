...
interface NewsItem {
  id: string; title: string; summary: string; url: string; tag: string;
  publishedAt: string; isPinned: boolean; isBreaking: boolean;
  source: { name: string };
}
    ...
    <div className="py-6">
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Search Global News..." value={search}
      ...
      {loading ? (
        <div className="text-center py-12 text-gray-400">Loading global news...</div>
      ) : items.length === 0 ? (
            ...
            <div key={item.id}
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    Source: {item.source.name} | {formatDate(item.publishedAt)}
                  </p>
                </div>
              ...
              {expanded.has(item.id) && (
                <div className="mt-3 pt-3 border-t border-navy-700">
                  <p className="text-sm text-gray-300 leading-relaxed">{item.summary || "No summary available."}</p>
